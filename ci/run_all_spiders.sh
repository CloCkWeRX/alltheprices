...
do
    echo "timeout -k 15s 8h uv run scrapy crawl --output ${SPIDER_RUN_DIR}/output/${spider}.json:json --logfile ${SPIDER_RUN_DIR}/logs/${spider}.txt --loglevel ERROR --set TELNETCONSOLE_ENABLED=0 --set CLOSESPIDER_TIMEOUT=${SPIDER_TIMEOUT} --set LOGSTATS_FILE=${SPIDER_RUN_DIR}/stats/${spider}.json ${spider}" >> ${SPIDER_RUN_DIR}/commands.txt
done
#  --output ${SPIDER_RUN_DIR}/output/${spider}.parquet:parquet
...
OUTPUT_LINECOUNT=$(cat "${SPIDER_RUN_DIR}"/output/*.json | wc -l | tr -d ' ')
...
(>&2 echo "Saving embed to https://data.alltheprices.xyz/runs/latest/info_embed.html")
...
if [ -z "${BUNNY_API_KEY}" ]; then
    ...
    curl --request GET \
         --silent \
         --url 'https://api.bunny.net/purge?url=https%3A%2F%2Falltheprices.b-cdn.net%2Fruns%2Flatest.json&async=false' \
    ...
    curl --request GET \
         --silent \
         --url 'https://api.bunny.net/purge?url=https%3A%2F%2Falltheprices.b-cdn.net%2Fruns%2Fhistory.json&async=false' \
         --header "AccessKey: ${BUNNY_API_KEY}" \
         --header 'accept: application/json'
    ...
    curl --request GET \
         --silent \
         --url 'https://api.bunny.net/purge?url=https%3A%2F%2Falltheprices.b-cdn.net%2Fruns%2Flatest%2Foutput%2F%2A&async=false' \
         --header "AccessKey: ${BUNNY_API_KEY}" \
         --header 'accept: application/json'
