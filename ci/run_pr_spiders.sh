...
pr_file_changes=$(curl -sL --header "authorization: token ${github_access_token}" "https://api.github.com/repos/CloCkWeRX/alltheprices/pulls/${pull_request_number}/files")
...
do
    ...
    LOGFILE_URL="https://alltheprices-data.openaddresses.io/ci/${CODEBUILD_BUILD_ID}/${SPIDER_NAME}/log.txt"
    ...
    if [ -f "$OUTFILE" ]; then
        ...
        if [ $FEATURE_COUNT == "0" ]; then
            ...
            PR_COMMENT_BODY="${PR_COMMENT_BODY}|[\`$spider\`](https://github.com/CloCkWeRX/alltheprices/blob/${GITHUB_SHA}/${spider})| (No Output) |Resulted in a \`${FAILURE_REASON}\` ([Log](${LOGFILE_URL}))|\\n"
            EXIT_CODE=1
            continue
        fi
        ...
        if [ ! -f "${STATSFILE}" ]; then
            ...
            if [ $num_errors -gt 0 ] || [ $num_warnings -gt 0 ]; then
                ...
                PR_COMMENT_BODY="${PR_COMMENT_BODY}|[\`$spider\`](https://github.com/CloCkWeRX/alltheprices/blob/${GITHUB_SHA}/${spider})|[${FEATURE_COUNT} items](${OUTFILE_URL}) ([Map](https://alltheprices.xyz/preview.html?show=${OUTFILE_URL}))|<details><summary>Resulted in a \`${FAILURE_REASON}\` ([Log](${LOGFILE_URL})) 🚨${num_errors} ⚠️${num_warnings}</summary><ul>${STATS_ERRORS}${STATS_WARNINGS}</ul></details>|\\n"
            else
                PR_COMMENT_BODY="${PR_COMMENT_BODY}|[\`$spider\`](https://github.com/CloCkWeRX/alltheprices/blob/${GITHUB_SHA}/${spider})|[${FEATURE_COUNT} items](${OUTFILE_URL}) ([Map](https://alltheprices.xyz/preview.html?show=${OUTFILE_URL}))|Resulted in a \`${FAILURE_REASON}\` ([Log](${LOGFILE_URL})) ✅|\\n"
            fi
            continue
        fi
...
if [ "${pull_request_number}" != "false" ]; then
    curl \
        ...
        "https://api.github.com/repos/CloCkWeRX/alltheprices/issues/${pull_request_number}/comments" > /dev/null
    ...
    echo "Added a comment to pull https://github.com/CloCkWeRX/alltheprices/pull/${pull_request_number}"
else
